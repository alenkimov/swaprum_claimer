...
async def _create_and_write(session, file, ref=None):
    account, code = await create_swaprum_account(session, ref=ref)
    account_data = ":".join((account.address, w3.to_hex(account.key), f"/active {code}"))
    file.write(account_data)
    file.write("\n")


async def create_accounts(count, ref=None):
    logger.info(f"Creating {count} accounts..")
    with open(ACCOUNTS_TXT, "w") as file:
        async with aiohttp.ClientSession() as session:
            tasks = []
            for i in range(count):
                tasks.append(asyncio.create_task(_create_and_write(session, file, ref=ref)))
